...
# Chapter 3. Heterogeneous Treatment Effect Estimation
...
\begin{equation}
Y_i = T_i Y_i^{(1)} + (1 - T_i) Y_i^{(0)}. \label{eq:obsY}
\end{equation}
...
\begin{equation}
(Y^{(1)}, Y^{(0)}) \perp T \mid X, \label{eq:assump2}
...
\begin{equation}
0 < P(T_i = 1 | X_i = x) =\pi(x) < 1 \label{eq:assump3}
\end{equation}<!-- \tag{Assumption 3} --> 
which in combine called strong ignorability. Assumption 3 is a common support regularization which means that in every corner of the covariate space, no arm will dominate. This assumption is often required in matching, weighting, and sub-classification related approaches to avoid the extreme value of propensity score (will be formally introduced in the next section).
...
\begin{equation}
\tau = E_X\{ E[Y\mid T = 1, X] - E[Y\mid T = 0, X]\}, \label{eq:ATE1}
\end{equation}
or
\begin{equation}
\tau = E \left[\frac{TY}{\pi(X)} \right] - E \left[\frac{(1 - T)Y}{1-\pi(X)} \right] = E\left[ \frac{T-\pi(X)}{\pi(X)(1-\pi(X))}Y \right], \label{eq:ATE2}
\end{equation}
...
\begin{equation}
\tau(x) =  E[Y\mid T = 1, X=x] - E[Y\mid T = 0, X=x], \label{eq:CATE1}
\end{equation}
or
\begin{equation}
\tau(x) = E \left[\frac{TY}{\pi(X)} \;\middle|\; X = x \right] - E \left[\frac{(1-T)Y}{1-\pi(X)} \;\middle|\; X=x \right] = E\left[ \frac{T-\pi(X)}{\pi(X)(1-\pi(X))}Y \;\middle|\; X = x\right].  \label{eq:CATE2}
\end{equation}
In some literature, the treatment indicator is denoted as $W = 2T-1 \in \{-1,1\}$, then equation $\eqref{eq:ATE2}$ and $\eqref{eq:CATE2}$ could be simply expressed by
\begin{align}
\tau &= E\left[ \frac{WY}{W\pi(X) + (1-W)/2} \right],  \label{eq:ATE3} \\
\tau(x) &= E\left[ \frac{WY}{W\pi(X) + (1-W)/2} \;\middle|\; X = x \right].  \label{eq:CATE3}
\end{align}

Almost every work aiming at finding ATE or CATE start from one of the above equations. For example, equation $\eqref{eq:ATE1}$ and $\eqref{eq:CATE1}$ are the foundation of conditional mean response methods, whereby the target is transformed to estimating conditional mean response $\mu_k(x) = E[Y \mid T = k, X=x], k \in \{0,1\}$. Equation $\eqref{eq:ATE2}$, $\eqref{eq:CATE2}$, $\eqref{eq:ATE3}$, and $\eqref{eq:CATE3}$ are the prototype of inverse probability weighted (**IPW**) methods which have been frequently used in modified outcome methods (**MOM**) or modified covariate methods (**MCM**). For instance, if both arm are equally allocated, that is $\pi = 0.5$, then equation $\eqref{eq:CATE3}$ turns out to be
$$
...
$$
udner assumption 2. Since then, the whole matching task is transformed from a high dimensional space to a linear one, but how to achieve a good estimator of propensity score $\pi(X)$ turns out to be the key problem. 
...
which does not hurt a lot by the missed values along $\pi(x)$. However, in estimation of CATE, there will be a lot of 'holes' in the estimators since it could be infinite numbers between $[0,1]$, and it is then less likely to do the prediction.
  ...
  @hirano2003efficient borrow the strength of equation $\eqref{eq:ATE2}$ to directly estimate the ATE which only need to plug in the pre-estimated propensity score $\hat\pi(X)$ without matching and subclassification
\begin{equation}
\tau = \frac{1}{|\mathcal{D}|} \sum_{i \in \mathcal{D}}\left[ \frac{T_i-\hat\pi(X_i)}{\hat\pi(X_i)(1-\hat\pi(X_i)}Y_i \right]. \label{eq:hirano}
\end{equation}
...
\begin{equation}
\mathbb{E}[l(\hat\tau, \tau)] = \frac{1}{|\mathcal{D}|}\sum_{i \in \mathcal{D}} \left(\hat\tau(x_i) - \tau(x_i) \right)^2,  \label{eq:Loss}
\end{equation}
...
Another set of straightforward methods follow directly from equation $\eqref{eq:CATE1}$ that the estimation of CATE $\tau(X)$ is accomplished by the estimation of the conditional means $\mu_k(X), k \in \{0,1\}$
$$
...
The main idea is that even though we cannot observe $\tau(X)$ directly, we could replace the unknown $\tau(X)$ in equation $\eqref{eq:Loss}$ by some available proxies with the property that minimizing this new loss function is equivalent to minimizing the target loss function $\eqref{eq:Loss}$. Most methods to discuss in this subsection are in the category of modified variate methods (MVM), which consists of modified outcome methods (MOM) and modified covariate method (MCM). 
...
\begin{equation}
E\left[ \{\hat\tau(X)-2WY \}^2 \right]. \label{eq:MOMraw}
\end{equation}
...
\end{equation}
if the outcome $Y$ is continuous. But $\tau(X) = E[2WT]$ holds only in 1\:1 randomized experiments, an IPW version of MOM is designed for observational studies. Based on equation $\eqref{eq:CATE2}$, the loss function for IPW version MOM is very straightforward
\begin{equation}
E[l(\hat\tau, \tau)] = E\left[ \left\{ \hat\tau(X)- \frac{T - \pi(X)}{\pi(X)(1-\pi(X))}Y \right\}^2 \right], \label{eq:MOMipw}
\end{equation}
where $\pi(X)$ is usually pre-estimated separately. Just as other IPW based methods, the results are sensitive to propensity score and no longer consistent when the model of $\pi(\cdot)$ is mis-specified [@knaus2018machine]. 
  ...
  Similarly, adopting equation $\eqref{eq:DR}$, we can develop the DR version of MOM. We do not further explore the possible proxies here. The main issue for MOM is that the outcome is required to be continuous and cannot be extended to a various type of outcomes. Therefore, MCM methods are proposed to fix this issue.
...
and then minimizing the right-hand-size, which is the loss function for MCM, targeting on $\hat\tau(X)$ directly as well. This branch of methods are fundamentally similar to MOM but do not make modification on the outcome, resulting in no restriction on $Y$. If under the linear assumption of $\hat\tau(X) = X^T\beta$, the original covariates $X$ is then turned to be $X/c$, and this is where the name modified covariate comes from. Furthermore, the CATE can be easily obtained using weighted least square methods, where $c^2$ is the weights.
  ...
  For example, the MCM version of the loss function $\eqref{eq:MOMraw}$ is thus
$$
E\left[ \{\hat\tau(X)\cdot W/2-Y \}^2 \right].
$$
For MOM-IPW version, with some modification on $\eqref{eq:MOMipw}$, the loss function for MCM-IPW version is derived as
$$
...
$$
in @abrevaya2015estimating. Then they plug this estimator into equation $\eqref{eq:CATE2}$, $\hat\tau(x)$ can be obtained by locally weighted approach. For direct loss metric, @chen2017general adopted B-spline based additive model to estimate $\hat\tau(x)$. $L_1$ penalty is also commonly used in kernel based methods to control the complexity of the models. 
...
## 3.3 Proposed Method {#sec3.13}
The current methods mainly discuss the scenario of two arms, treatment versus control. Even though in many literature, authors claim that their methods could be extended to multiple treatment cases, none of them really implemented it. Our proposed method is tailored for such complicated scenario and at the same time provide an alternative approach to the existing methods. Following the structure in [Section 3.2](#sec3.2), the proposed method will also be explored in two parts: loss metric and estimation algorithm. 

Following the notation in [Section 3.1](#sec3.1), assuming there are a total of $K+1$ treatments, $T\in\{0,1,2,...,K\}$ and denote $T=0$ the standard therapy or placebo. The covariate-specific causal effect for the $k^{th}$ treatment against standard therapy is
$$
\tau^{(k)}(X) = E[Y^{(k)} - Y^{(0)}|X],
$$
...
\begin{equation}
\arg\min_\tau \left\{ E\left[ \left(Y - m(X) - \sum_{k=1}^{K} \left(1[T=k] - \pi^{(k)}(X)\right) \tau^{(k)}(X) \right)^2 \right] \right\}. \label{eq:Rloss}
\end{equation}



### 3.3.2 Proposed Estimation Algorithm
To accomplish the optimization, we adopt a two-step estimation process according to the shape of the loss function. First we need to estimate $m(X)$ and $\pi^{(\cdot)}(X)$, and then plug the estimators into equation $\eqref{eq:Rloss}$. Besides, we need to propose a set of functions for the causal effects $\hat\tau^{(\cdot)}(X)$.
...
\begin{equation}
(\underline{\hat{\gamma}}, \underline{\hat{c}}) = \arg\min_{\underline\gamma, \underline c} \sum_{i=1}^n  \left(Y_i - \hat{m}(X_i) - \sum_{k=1}^{K} \left(1[T_i=k] - \hat\pi^{(k)}(X_i)\right) \tau^{(k)}(X_i;\underline\gamma, \underline c) \right)^2 . \label{eq:opt}
\end{equation}
The estimates of covariate-specific causal effects are then given by
\begin{equation}
\hat\tau^{(k)}(X) = \hat\gamma^{(k)} + \sum_{p = 1}^P\left(\sum_{l=1}^{q_n}\hat{c}^{(k)}_{lp}B_l(X_p)\right). \label{eq:tau}
\end{equation}

Instead of direct optimizing the loss function $\eqref{eq:opt}$ with LASSO algorithm just as in Nie's work, we adopt a tree-based recursive partition algorithm to figure out the optimal causal treatment effect. The idea is to recursively split the node to grow a tree which could achieve the homogeneous leaves after completion and within each node, we estimate the treatment effects by equation $\eqref{eq:tau}$. The details of generating trees will be further demonstrated in Section 3.3.3. 

The main reason to adopt tree-based algorithm is the natural feature of tree-based algorithm that generate homogeneous leaves and assign heterogeneous units to different leaves perfectly matches our requirement of seeking heterogeneous treatment effects. Our ultimate goal is to find out the optimal covariate specific treatment, rather than figuring out the most accurate estimation of treatment effects, which is almost impossible. So we prefer not to directly solve equation $\eqref{eq:opt}$ by LASSO but to subclassify homogeneous patients.
...
### 3.4.2 Potential Problems of EMR Data Application
...
## 3.5 Timeline
The future arrangement of time is shown in table $\ref{tab:tab2}$. Since many potential problems and difficulties will occur during the process, this schedule may not be able to follow strictly. Besides, the hypothecical tasks listed below is the based on current status and understanding. New ideas may also get inspired during the research and further exploration of the novelty and collaboration with committee members is encouraging. 
```{r, table2, echo = F}
col.name = paste('Task', seq(1,9))
row.name = c('11.2019','12.2019', paste(seq(1,12), '2020', sep = '.'))
tasks = matrix(data = '', nrow = length(row.name), ncol = length(col.name))
colnames(tasks) = col.name
rownames(tasks) = row.name
tasks[1,c(1,2)] = 'D'; 
tasks[c(1,2,3),3] = 'X'; tasks[c(2,3),4] = 'X'; tasks[c(3,4,5),5] = 'X'; tasks[c(5,6),6] = 'X'; 
tasks[c(7),7] = 'X'; tasks[c(7,8,9),8] = 'X'; tasks[c(9,10,11),9] = 'X'; 
kable(tasks, format = 'latex', align = 'c', booktabs = T, caption = "\\label{tab:tab2} Monthly Schedule") %>%
  footnote(general = c("D: Finished; X: Uncompleted", 
                       "Task 1: Literature review", 
                       "Task 2: Write proposal", 
                       "Task 3: Write manuscript for publication on first topic", 
                       "Task 4: Replicate the work of major current studies by simulation",
                       "Task 5: Develop and optimize the proposed algorithm",
                       "Task 6: Compare the simulation results with current method",
                       "Task 7: Application of antihypertensives data",
                       "Task 8: Write manuscript for publication on second topic",
                       "Task 9: Compose the final thesis document"
                       ), threeparttable = F)
```
...
\newpage
# Appendix {#App}
Proof of Equation $\eqref{eq:ATE1}$:
$$
\begin{aligned}
...
\end{aligned}
$$
Proof of Equation $\eqref{eq:ATE2}$:
$$
\begin{aligned}
\tau 
