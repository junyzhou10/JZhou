...
author: Junyi Zhou
date: "2019-11-05"
description: 'Review of Causal Inference: Heterogeneous Treatment Effect Estimation (old version)'
categories:
- Lectures/Slides
tags:
- Causal Inference
title: 'Review of Causal Inference: Heterogeneous Treatment Effect Estimation (old version)'
...
*Notice: the causal inference described here follows the idea of Potential Outcome Framework [@neyman1923application; @rubin1974estimating]. For another big camp of causal inference studies, i.e., Structural Causal Model (SCM) [@pearl2009causality], please refer to this well-written paper [here](https://ftp.cs.ucla.edu/pub/stat_ser/r350.pdf).*
...
\xi_i = Y_i^{(1)} - Y_i^{(0)}.
\end{equation*}
Clearly, the definition of causal effect does not depend on which potential outcome is observed. Besides, ITE should be measured on the same unit, at same time point post-treatment. A direct observation and measurement of ITE is therefore infeasible. However, we are able to estimate the average individual treatment effect under plausible assumptions, which will be discussed in subsection [1.2](#sec3.1.2). Here we first denote expected ITE for a specific unit $i$ with a vector of covariates $X_i$, or individual average treatment effect (IATE), as
$$
...
### 1.2 Observational Studies{#sec3.1.2}
...
Note that only the linear structure on $\hat\tau(X)$ in MVM based methods [@tian2014simple; @nie2017quasi; @knaus2018machine] has been studied in the literature due to the simplicity and ease in computation, though it is limited in scope. For more complicated proposed structure of $\hat\tau(X)$, general optimization algorithms could be directly applied on these loss functions. 

##### R-learner
R-learner [@nie2017quasi] is the only method that does not directly rely on equation \@ref(eq:ATE1) - \@ref(eq:CATE3). Technically, however R-learner can still be regarded as a MOM based method. We highlight it here because it is different from previous methods and more importantly, it builds the technical foundation of our proposed method. R-learner method mainly bases on Robinson's [@robinson1988root] transformation 
$$
...
Suppose there are $P$ covariates, then the conditional mean outcome could be modeled by
$$
