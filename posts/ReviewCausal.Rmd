...
Different scientific questions leads to the estimation of different causal estimands. As aforementioned, for instance, ATE is basically for assessing the population/sub-population treatment effect which is the key focus for the drug companies. On the other hand, CATE which is more about individual-level treatment effect, so it can be used to do precision medicine or personalized recommendations. Here is a list of the acronym of some commonly used causal estimands:
...
### Matching(subclassification/coarsened) {#Matching}
...
which leads to
\begin{equation}
\tau = E[Y^{(1)} - Y^{(-1)}] = E\left[\frac{1[T = 1]Y}{\pi(\mathbf X)}\right] - E\left[\frac{1[T = -1]Y}{1-\pi(\mathbf X)}\right].  (\#eq:IPW)
\end{equation}
...
&- \underbrace{E\left[\frac{1[T=1] -\pi(\mathbf X)}{\pi(\mathbf X)}\mu_1(\mathbf X) + \frac{1[T=1] -\pi(\mathbf X)}{1-\pi(\mathbf X)} \mu_{-1}(\mathbf X)\right]}_\text{Augmentation} (\#eq:AIPW)
\end{align}
...
### Covariate Balancing Propensity Score (CBPS) {#CBPS}
...
- Weights (or propensity score) estimated on purposely for covariate balancing, e.g., [CBPS](#CBPS) [@imai2014covariate], Entropy balancing [@hainmueller2012entropy], EBCM (empirical balancing calibration weighting) [@chan2016globally], etc. (Mostly, PS/weights obtained in this approach are mainly for weighting method)
...
- [Matching](#Matching) [@rosenbaum1983central]
- Subclassification [@rosenbaum1984reducing]
- Weighting
  + HT [@horvitz1952generalization]
  + [IPW](#IPW) (IPTW) [@hirano2003efficient]
- Regression
...
- Mixture of above
  + [AIPW](#AIPW) [@bang2005doubly]
  + [TMLE](#TMLE) [@van2006targeted]
...
```{r prediag, echo=FALSE, fig.width=10, fig.height=3.2, fig.cap="\\label{fig:prediag}Illustration of predictive and diagnostic variable"}
...
\begin{equation}
\hat\tau(\mathbf X) = \hat\mu(\mathbf X, 1) - \hat\mu(\mathbf X, -1). (\#eq:Slearner)
\end{equation}
...
\begin{equation}
\hat\tau(\mathbf X) = \hat\mu_1(\mathbf X) - \hat\mu_{-1}(\mathbf X) (\#eq:Tlearner)
\end{equation}
...
\begin{equation}
\tau(\mathbf x) = E\left[ \frac{TY}{T\pi(\mathbf X) + (1-T)/2} \;\middle|\; \mathbf  X = \mathbf x \right] (\#eq:OWIPW)
\end{equation}
...
\begin{equation}
E[l(\hat\tau, \tau)] = E\left[ \left\{ \hat\tau(\mathbf X)- \frac{TY}{T\pi(\mathbf X) + (1-T)/2} \right\}^2 \right] (\#eq:MOMIPW)
\end{equation}
...
Most of the previous methods target on CATE estimation, whereas the optimal treatment can be found automatically by looking at the estimates. But the fact is, obtaining the consistent and unbiased estimates for CATE is difficult. However, if we only need to know which treatment is the optimal, we actually do not need to estimate the CATE but only the treatment assignment rule, or so-called individual-treatment-rule (ITR), $d(\mathbf x) = sgn\{\tau(\mathbf x)\}$, which is a mapping $d: \mathcal X \rightarrow T$. In practice, we do not need to estimate $\tau(\cdot)$ but $d(\cdot)$ directly. See Figure (\@ref(fig:ITR)) for a demonstration of the difference between Q-learning, A-learning, and ITR.


```{r ITR, echo=FALSE, fig.width=10, fig.height=3.2, fig.cap="\\label{fig:ITR}Illustration of causal inference approaches. (A) Q-learning aims at estimating the response surfaces of each treatment group; (B) A-learning aims at estimation of treatment effect directly; (C) ITR aims at estimation of treatment region rather than the treatment effect."}

set.seed(12345)
x  = seq(0.01,1,length.out = 100)
Y1 = 0.5*log(x)+1
Y2 = ifelse(x<0.4, -0.2, -0.2+4*(x-0.4))
layout(matrix(seq(1,3), nrow = 1))
plot(x, Y1, ylim = range(Y1, Y2), type = "l", lwd = 2, col = "darkred", main = "(A) Q-learning", xlab = "Covariate", ylab = "Outcome")
lines(x, Y2, lwd = 2, col = "darkblue")
legend("topleft", legend = c("E(Y | T = 1)", "E(Y | T = -1)"), col = c("darkred", "darkblue"), lwd = c(2,2))

plot(x, Y1-Y2, type = "l", lwd = 2, col = "darkgreen", main = "(B) A-learning", xlab = "Covariate", ylab = "Treatment Effect")
abline(h = 0, lty = 2, col = "gray40")
legend("topright", legend = expression(tau), col = "darkgreen", lwd = 2)

plot(x, Y1-Y2, type = "n", lwd = 2, col = "gray40", main = "(C) ITR", lty = 2, xlab = "Covariate", ylab = "", yaxt = 'n')
segments(0, 0, 0.09,   0, col = "darkblue", lwd = 5)
segments(0.09, 0, 0.645,   0, col = "darkred", lwd = 5)
segments(0.64, 0, 1,   0, col = "darkblue", lwd = 5)
legend("topleft", legend = c("T = 1", "T = -1"), col = c("darkred", "darkblue"), lwd = c(5,5))
# the root above can be found by:
# uniroot(function(X1) 1 + 0.5*log(X1) - ifelse(X1<0.4, -0.2, -0.2+4*(X1-0.4)), interval = c(0,0.5))
# uniroot(function(X1) 1 + 0.5*log(X1) - ifelse(X1<0.4, -0.2, -0.2+4*(X1-0.4)), interval = c(0.5,1))
```

Recall the ITR is indeed a mapping from the covariate space into the treatment space $d: \mathcal X \rightarrow T$. According to @qian2011performance and @zhao2012estimating, the value function under the ITR $d(\mathbf X)$ is
$$
...
d^{*}() = \arg\max_{d()} V(d).
$$
Notice that solving this objective function requires discontinuous loss functions, which may incur certain numerical difficulty. So far, various methods have been proposed in the literature to obtain the ITR following this idea [@qian2011performance; @zhao2012estimating; @xu2015regularized; @zhu2017greedy; @zhou2017residual; @zhang2018c; @qi2020multi]. Due to its structure similar to propensity score weighting but only on outcome $Y$, it is also known as outcome weighted learning, or O-learning [@zhao2012estimating].
...
(For more details on CATE/HTE estimation, please refer to another post [Meta-Learners](https://jzhou.org/posts/metalearner/))
